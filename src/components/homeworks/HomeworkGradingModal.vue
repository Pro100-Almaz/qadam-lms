              ...
              >
                ...
                <div class="mt-1.5 pl-11">
                  <input
                    ...
                    type="text"
                    maxlength="500"
                    :aria-label="t('homeworks.commentFor', { name: row.fullName })"
                    :placeholder="t('homeworks.commentPlaceholder')"
...
interface GradeRow {
  ...
  original: string
  /** Optional teacher note stored alongside the mark. */
  comments: string
  /** Trimmed comment as loaded, for change detection. */
  ...
  const results = await Promise.allSettled(
    changed.map(row => {
      ...
      return row.gradeId === null
        ? createHomeworkGradeApi(homework.id, { student: row.studentId, grade, comments })
        : updateHomeworkGradeApi(row.gradeId, { grade, comments })
    }),
  )
  saving.value = false
